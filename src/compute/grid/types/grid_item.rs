...
pub(in super::super) struct GridItem {
    ...
    /// Ancestor margin and gutter-delta space projected into final positioning for subgridded axes.
...
impl GridItem {
    fn preferred_size_constraint(&self, axis: AbstractAxis) -> Option<AvailableSpace> {
        match self.size.get(axis).tag() {
    ...
    pub fn definite_grid_area_size_in_axis(
        ...
        axis_tracks[self.track_range_excluding_lines(axis)]
            ...
            .map(|track| {
                let min_size =
                    track.min_track_sizing_function.definite_value(axis_parent_size, resolve_calc_value)?;
                let max_size =
                    track.max_track_sizing_function.definite_value(axis_parent_size, resolve_calc_value)?;
    ...
    fn known_dimensions(
        ...
        let padding = Self::resolved_axis_side_lengths(self.padding, grid_area_size.width, |val, basis| {
            tree.calc(val, basis)
        });
        let border = Self::resolved_axis_side_lengths(self.border, grid_area_size.width, |val, basis| {
            tree.calc(val, basis)
        });
        let padding_border_size = (padding + border).sum_axes();
        let box_sizing_adjustment =
            if self.box_sizing == BoxSizing::ContentBox { padding_border_size } else { Size::ZERO };
        let preferred_inline_constraint = self.preferred_size_constraint(AbstractAxis::Inline);
        let preferred_block_constraint = self.preferred_size_constraint(AbstractAxis::Block);
    ...
    fn measure_content_sized_preferred_axis(
        &self,
        axis: AbstractAxis,
        tree: &mut impl LayoutPartialTree,
        grid_area_size: Size<Option<f32>>,
        ...
        let preferred_constraint = self.preferred_size_constraint(axis)?;

        let mut known_dimensions = self.known_dimensions(tree, grid_area_size);
        known_dimensions.set(axis, None);

        let mut preferred_available_space = available_space;
        preferred_available_space.set(axis, preferred_constraint);

        let contribution = tree.measure_child_size(
            self.node,
            known_dimensions,
            grid_area_size,
            preferred_available_space,
            SizingMode::InherentSize,
            axis.as_abs_naive(),
            Line::FALSE,
        ) - self.subgrid_self_duplicate_contribution(axis, tree, inner_node_size);

        Some(contribution.max(self.empty_subgrid_padding_border_floor(axis, tree, inner_node_size)))
    }
    ...
    pub fn min_content_contribution(
        ...
        let available_space = available_space.map(|opt| match opt {
            Some(size) => AvailableSpace::Definite(size),
            None => AvailableSpace::MinContent,
        });

        if let Some(contribution) =
            self.measure_content_sized_preferred_axis(axis, tree, grid_area_size, available_space, inner_node_size)
        {
            return contribution;
        }
    ...
    pub fn min_content_contribution_cached(
        ...
        self.min_content_contribution_cache.get(axis).unwrap_or_else(|| {
            let size = self.min_content_contribution(axis, tree, grid_area_size, available_space, inner_node_size);
    ...
    pub fn max_content_contribution_cached(
        ...
        self.max_content_contribution_cache.get(axis).unwrap_or_else(|| {
            let size = self.max_content_contribution(axis, tree, grid_area_size, available_space, inner_node_size);
    ...
    pub fn minimum_contribution(
        ...
        let box_sizing_adjustment =
            if self.box_sizing == BoxSizing::ContentBox { padding_border_size } else { Size::ZERO };
        let size = if self.preferred_size_constraint(axis).is_some() {
            Some(self.min_content_contribution_cached(axis, tree, grid_area_size, grid_area_size, inner_node_size))
        } else {
            self.size
                .maybe_resolve(grid_area_size, |val, basis| tree.calc(val, basis))
            ...
            .unwrap_or_else(|| {
                ...
                if use_content_based_minimum {
                    let mut minimum_contribution =
                        self.min_content_contribution_cached(axis, tree, grid_area_size, grid_area_size, inner_node_size);
